  ...
  class Collection < Lookbook::Object
    ...
    protected def load!
      ...
      @entities = if @entities
        Lookbook.update(@entities)
      else
        Lookbook.analyze(path)
      end
    end
    ...
    class << self
      ...
      def load_from_config
        collections = config.collections.presence || [{name: "default", path: "lookbook"}]

        @collections = collections.map do |config_opts|
          options = config_opts.slice(*literal_properties.map(&:name)).deep_symbolize_keys
          new(config_opts[:name].to_sym, **options)
        end || []
      end
      ...
      def root_path
        @root_path ||= (defined?(Rails) && Rails&.root.present?) ? Rails.root : Dir.pwd
      end
